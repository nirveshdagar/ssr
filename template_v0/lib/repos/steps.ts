...
/**
 * "Force ONE step" reset: clear ONLY step N's row to 'pending'. Steps
 * BEFORE N stay (they're upstream — completed/skipped reflects reality).
 * Steps AFTER N also stay completed/skipped, so the pipeline's per-step
 * lock causes them to be silently skipped instead of being re-run.
 * Used by the dashboard's "Force" button on a completed/skipped step
 * when the operator wants to retry exactly one step without churning
 * the later already-finished work.
 */
export function resetSingleStep(domain: string, stepNum: number): void {
  run(
    `UPDATE step_tracker
        SET status='pending', message='', started_at=NULL, finished_at=NULL
      WHERE domain = ? AND step_num = ?`,
