...
/**
 * Defensive unescape — handles models that double-escape their JSON output.
 *
 * Symptom: apppilot.site was generated by a model whose response, after our
 * single JSON.parse, still contained literal `\n` / `\"` / `\\` byte
 * sequences instead of real newlines / quotes / backslashes. The file was
 * then written verbatim to /public_html/index.php and the browser saw
 * broken HTML.
 *
 * Heuristic: HTML content always has real newlines. If we see `\n` or
 * `\"` as literal characters AND no real newlines in a long-ish string,
 * the LLM double-escaped its JSON output. Decode common escape sequences,
 * being careful not to chain-decode `\\` → `\` → start-of-escape.
 */
function unescapeDoubleEncodedHtml(s: string): string {
  ...
  // Use a placeholder for `\\` so we don't decode `\\n` → `\n` → newline
  // (which would lose a literal backslash that the LLM intended).
  const PLACEHOLDER = " "
  return s
...
async function _generateSinglePageImpl(
  ...
  if (parsed.files !== undefined) {
    const v0 = validateGeneratedFiles(parsed.files)
    if ("error" in v0) {
      throw new Error(`Generator returned invalid files array: ${v0.error}`)
    }
    // Apply the double-escape recovery to every file's content. See
    // unescapeDoubleEncodedHtml — silent no-op when the content was
    // already decoded properly.
    const v = v0.map((f) => ({ ...f, content: unescapeDoubleEncodedHtml(f.content) }))
  ...
  const phpRaw = String((parsed.php ?? parsed.html ?? "") as string)
  const php = unescapeDoubleEncodedHtml(phpRaw)
