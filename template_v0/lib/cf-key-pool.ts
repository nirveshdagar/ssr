...
export function getNextAvailableCfKey(): CfKeyWithCreds {
  // Pick by natural alias order (CF1 → CF2 → CF3 → … → CF10 → CF11 → CF100),
  // not by insertion-order id, so the operator can predict which key picks up
  // a new domain. SQLite has no built-in natural collation; cheap to fetch all
  // eligible rows + sort in JS — the active pool is at most a few thousand.
  const rows = all<CfKeyWithCreds>(
  ...
  if (rows.length > 0) {
    rows.sort((a, b) => {
      const labelA = a.alias || a.email
      const labelB = b.alias || b.email
      const cmp = labelA.localeCompare(labelB, undefined, {
        numeric: true, sensitivity: "base",
      })
      // Stable tiebreaker on id so a duplicate-alias edge case still
      // picks deterministically (older row first).
