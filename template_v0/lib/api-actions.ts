...
export const domainActions = {
  ...
  runFromStep: async (
    ...
    opts: {
      ...
      /** Force EXACTLY this step (the dashboard's "Force" button on a
       *  completed step). Steps AFTER N stay locked at completed and the
       *  pipeline silently skips them. Without this, the call behaves
       *  as "Run from here" — reset step N AND every step after to
       *  pending so the run propagates forward. */
      onlyThis?: boolean
    } = {},
    ...
    try {
      const r = await fetch(`/api/domains/${domain}/run-from/${step}`, {
        ...
        body: JSON.stringify({
          ...
          only_this: opts.onlyThis ? "on" : undefined,
        }),
      })
      const j = (await r.json().catch(() => ({}))) as Record<string, unknown>
      return {
