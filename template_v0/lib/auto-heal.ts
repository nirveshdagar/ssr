...
// ---------------------------------------------------------------------------
// Force-HTTPS drift sweep — catches the case where the origin cert is right
// (CF Origin CA) but the port-80 → HTTPS redirect got turned off (e.g. an
// operator clicked "force_https off" in SA's UI, or an apache config edit
// removed the rewrite rule). Re-runs step 8 to re-add the redirect.
// ---------------------------------------------------------------------------

export interface ForceHttpsSweepResult {
  checked: number
  missing: { domain: string; serverIp: string; status: number | null; location: string | null }[]
  enqueuedReinstalls: { domain: string; jobId: number }[]
}

/**
 * For every `live`/`hosted` domain whose origin cert is verified
 * (`ssl_origin_ok=1`), probe HTTP→HTTPS redirect on origin port 80 with
 * SNI/Host=<domain>. If the redirect isn't there:
 *   - log + audit `force_https_missing`
 *   - enqueue `pipeline.full {start_from:8}` to re-run the SSL step
 *     (which now ensures the redirect via both API + SSH paths)
 *
 * Gating mirrors `checkOriginCerts`: only fires the auto-fix when
 * `settings.auto_migrate_enabled='1'`, capped at
 * `SSR_FORCE_HTTPS_MAX_AUTOFIX_PER_HOUR` (default 3, same shape as the
 * SSL cap) to prevent loops if step 8 keeps not adding the rewrite.
 *
 * Filter `ssl_origin_ok=1` so a domain with broken cert is driven by
 * `checkOriginCerts` instead — no double-enqueue.
 */
export async function checkForceHttpsRedirects(): Promise<ForceHttpsSweepResult> {
  const rows = all<{ domain: string; server_id: number; current_proxy_ip: string | null }>(
    `SELECT d.domain, d.server_id, s.ip AS current_proxy_ip
       FROM domains d
       JOIN servers s ON s.id = d.server_id
      WHERE d.status IN ('hosted','live')
        AND s.ip IS NOT NULL
        AND d.ssl_origin_ok = 1`,
  )
  const result: ForceHttpsSweepResult = { checked: 0, missing: [], enqueuedReinstalls: [] }
  if (rows.length === 0) return result
  ...
  for (const r of rows) {
    ...
    let probe
    try {
      probe = await probeForceHttps(r.current_proxy_ip, r.domain, 8000)
    } catch (e) {
      logPipeline(r.domain, "force_https", "warning",
        `Probe threw: ${(e as Error).message.slice(0, 160)}`)
      continue
    }
    if (probe.ok === true) {
      // Healthy. No log spam on the happy path — there's a lot of these.
      continue
    }
    if (probe.ok === null) {
      logPipeline(r.domain, "force_https", "warning",
        `Force-HTTPS probe inconclusive: ${probe.message}`)
      continue
    }
    // probe.ok === false → port 80 answered but didn't 301 to https://.
    result.missing.push({
      domain: r.domain, serverIp: r.current_proxy_ip,
      status: probe.status, location: probe.location,
    })
    logPipeline(r.domain, "force_https", "warning",
    ...
    if (autoFixEnabled) {
      ...
      if (recent >= MAX_PER_HOUR) {
        logPipeline(r.domain, "force_https", "warning",
          ...
          `for this domain until the redirect returns (rewrite injection isn't sticking).`)
        try {
          appendAudit(
            "force_https_autofix_giveup", r.domain,
            `count=${recent}/60min server_id=${r.server_id}`, null,
          )
        } catch { /* ignore */ }
        try {
          const { notify } = await import("./notify")
          await notify(
            `Force-HTTPS missing (${r.domain})`,
            `Auto-fix gave up after ${recent} step-8 re-runs in 60 min ` +
            `failed to add the HTTP→HTTPS redirect on server #${r.server_id} ` +
            `(${r.current_proxy_ip}). Probe shows: ${probe.message}. ` +
            `Likely an Apache vhost issue — check ${r.current_proxy_ip} manually.`,
            { severity: "warning", dedupeKey: `force_https_giveup:${r.domain}` },
          )
        } catch { /* notify is best-effort */ }
      } else {
        const inflight = all<{ id: number }>(
        ...
        if (inflight.length === 0) {
          ...
          result.enqueuedReinstalls.push({ domain: r.domain, jobId })
          logPipeline(r.domain, "force_https", "running",
            `Auto-fix: enqueued pipeline.full from step 8 (job ${jobId}) to re-add ` +
            `force-HTTPS redirect (attempt ${recent + 1}/${MAX_PER_HOUR}).`)
        } else {
          logPipeline(r.domain, "force_https", "warning",
            `Redirect missing but pipeline.full is already queued/running — ` +
            `skipping auto-fix to avoid stacking.`)
        }
      }
    }
    ...
    try {
      ...
      await notify(
        `Force-HTTPS missing: ${r.domain}`,
        `HTTP origin at ${r.current_proxy_ip} returned status=${probe.status ?? "?"} ` +
...
// ---------------------------------------------------------------------------
// SA tracker drift sweeper — origin wire is correct but SA's dashboard panel
// still shows "No SSL" or "Automatic" instead of "Custom · CloudFlare Origin".
// ---------------------------------------------------------------------------

export interface SaTrackerDriftResult {
  checked: number
  drifted: { domain: string; saSsl: string | null; saForce: number | null }[]
  enqueuedReinstalls: { domain: string; jobId: number }[]
}

/**
 * Catch the "SA UI stays blank even though the cert is on the wire" failure
 * mode (added 2026-05-14 after a fresh bulk run showed 4 of 5 domains with
 * `ssl_origin_ok=1` but SA's tracker stuck at `ssl=null`). `checkOriginCerts`
 * only fires on wrong-cert-on-wire and `checkForceHttpsRedirects` only fires
 * on missing port-80 redirect — neither catches tracker drift.
 *
 * For each `live`/`hosted`/`ssl_installed` domain with `ssl_origin_ok=1`,
 * read SA's own `app.ssl` + `app.force_ssl` flags (free from the existing
 * `listApplications` call — no per-domain GET /ssl needed). If the tracker
 * doesn't show `custom` + force_ssl=1, enqueue `pipeline.full {start_from:8}`
 * so step 8 + `pushSaUiTracker` re-run.
 *
 * Same gating shape as the other sweepers: auto-fix only when
 * `settings.auto_migrate_enabled='1'`, capped via
 * `SSR_SA_TRACKER_MAX_AUTOFIX_PER_HOUR` (default 3) to break loops.
 */
export async function checkSaTrackerDrift(): Promise<SaTrackerDriftResult> {
  const rows = all<{ domain: string; server_id: number; sa_server_id: string | null }>(
    `SELECT d.domain, d.server_id, s.sa_server_id
       FROM domains d
       JOIN servers s ON s.id = d.server_id
      WHERE d.status IN ('hosted','live','ssl_installed')
        AND d.ssl_origin_ok = 1
        AND s.sa_server_id IS NOT NULL`,
  )
  const result: SaTrackerDriftResult = { checked: 0, drifted: [], enqueuedReinstalls: [] }
  if (rows.length === 0) return result

  // Group by sa_server_id so we make ONE listApplications call per server.
  const bySrv = new Map<string, typeof rows>()
  for (const r of rows) {
    if (!r.sa_server_id) continue
    const list = bySrv.get(r.sa_server_id) ?? []
    list.push(r); bySrv.set(r.sa_server_id, list)
  }
  ...
  for (const [saServerId, domainRows] of bySrv) {
    let apps
    try {
      apps = await listApplications(saServerId)
    } catch (e) {
      logPipeline(`server-${saServerId}`, "sa_tracker_drift", "warning",
        `listApplications failed: ${(e as Error).message.slice(0, 160)}`)
      continue
    }
    type AppLite = { primary_domain?: string; name?: string; ssl?: string | null; force_ssl?: number | null }
    const byDomain = new Map<string, AppLite>()
    for (const a of apps as AppLite[]) {
      const k = (a.primary_domain ?? "").toLowerCase()
      if (k) byDomain.set(k, a)
    }

    for (const r of domainRows) {
      result.checked++
      const a = byDomain.get(r.domain.toLowerCase())
      if (!a) continue  // app not on SA (e.g., mid-teardown) — nothing to push
      const isCorrect = a.ssl === "custom" && a.force_ssl === 1
      if (isCorrect) continue

      result.drifted.push({ domain: r.domain, saSsl: a.ssl ?? null, saForce: a.force_ssl ?? null })

      // Quiet-mode check: once this domain has hit the per-hour cap AND
      // we've already announced "Auto-fix DISABLED" recently, stay
      // silent until something changes. Previously we re-logged BOTH
      // the per-tick "stale" warning AND the cap-hit announcement on
      // every 5-min sweep, producing 8 redundant warnings per drift
      // cycle for 4 stuck domains (observed during 2026-05-14 migration
      // settle).
      //
      // Window aligned with LE's 168h duplicate-cert window — the
      // longest realistic time a stuck-on-LE-rate-limit domain stays
      // stuck without operator intervention. After 7 days, if the
      // domain is STILL stuck, one re-up notification fires as a
      // "yes we still know about this" heartbeat. Operator-visible
      // state for in-between is the live dashboard, not pipeline_log.
      const recentDisabled = (one<{ n: number }>(
        `SELECT COUNT(*) AS n FROM pipeline_log
          WHERE domain = ? AND step = 'sa_tracker_drift' AND status = 'warning'
            AND message LIKE 'Persistent drift%'
            AND created_at >= datetime('now', '-168 hours')`,
        r.domain,
      )?.n ?? 0)
      const inQuietMode = recentDisabled > 0

      if (!inQuietMode) {
        logPipeline(r.domain, "sa_tracker_drift", "warning",
          `SA tracker stale: ssl=${a.ssl ?? "null"} force_ssl=${a.force_ssl ?? 0} ` +
          `but wire is verified CF Origin (ssl_origin_ok=1).`)
      }
      try {
      ...
      if (recent >= MAX_PER_HOUR) {
        ...
        // re-announce, which is fine: it signals the operator that
        // we're still in the disabled state.
        if (!inQuietMode) {
          logPipeline(r.domain, "sa_tracker_drift", "warning",
            `Persistent drift — ${recent} in last 60 min. Auto-fix DISABLED for ` +
            `this domain (likely LE 5-per-168h rate-limit; use manual paste).`)
        }
        continue
      }
      const inflight = all<{ id: number }>(
      ...
      result.enqueuedReinstalls.push({ domain: r.domain, jobId })
      logPipeline(r.domain, "sa_tracker_drift", "running",
        `Auto-fix: enqueued pipeline.full from step 8 (job ${jobId}) to refresh ` +
        `SA tracker (attempt ${recent + 1}/${MAX_PER_HOUR}).`)
    }
  }
  return result
}
...
export interface AutoHealTickResult {
  ...
  forceHttpsSweep: ForceHttpsSweepResult | { error: string }
  saTrackerDrift: SaTrackerDriftResult | { error: string }
  ...
  try {
    ...
    if ((getSetting("auto_heal_paused") || "").trim() === "1") {
      return {
        ...
        forceHttpsSweep: { error: "paused" },
        saTrackerDrift: { error: "paused" },
