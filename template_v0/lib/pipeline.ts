...
import {
  ...
  installAgentOnDroplet, isSaServerAlive, disableAutoSsl,
} from "./serveravatar"
...
async function pipelineWorkerImpl(
  ...
  try {
    ...
    if (startFrom == null || startFrom <= 2) {
      ...
      updateStep(domain, 2, "skipped", "start_from > 2")
    }
    ...
    if (startFrom == null || startFrom <= 3) {
      ...
      updateStep(domain, 3, "skipped", "start_from > 3")
    }
    ...
    if (startFrom == null || startFrom <= 4) {
      ...
      updateStep(domain, 4, "skipped", "start_from > 4")
    }

    checkCancel(domain)
    if (startFrom == null || startFrom <= 5) {
      if (shouldRun(5)) {
        // 30-min budget for NS propagation. Real-world CF zone-activation
        // after a registrar NS change typically lands in 5–30 min (rarely
        // up to 24h on stale resolvers). The old 2-min budget was too tight
        // — step 5 would warning-out, the pipeline would barrel through to
        // step 8 wasting a fresh droplet on step 6, then step 8 would throw
        // OriginCaZoneNotActiveError and bounce status back to ns_set,
        // leaving the operator dependent on auto-heal's 5-min sweep to
        // re-poll. Keep step 5 actively polling instead. Cancel-check is
        // inside the loop so the operator can still interrupt.
        await step5WaitZoneActive(domain, 1_800_000, 15_000)
      }
    } else {
      // Skipped because operator asked to start from a later step. Zone
      // is assumed already-active — without this `else`, the step_tracker
      // row stays at whatever it was previously (pending if re-added,
      // completed from a prior run), which made the dashboard wrongly
      // show "Wait for Zone Active" as not yet done on domains that
      // were live.
      updateStep(domain, 5, "skipped", "start_from > 5 — zone already active")
    }
    ...
    if (startFrom == null || startFrom <= 7) {
      ...
      updateStep(domain, 7, "skipped", "start_from > 7")
    }
    ...
    if (startFrom == null || startFrom <= 8) {
      ...
      updateStep(domain, 8, "skipped", "start_from > 8")
    }
...
async function step5WaitZoneActive(domain: string, timeoutMs = 600_000, pollMs = 30_000): Promise<boolean> {
  ...
  while (Date.now() < deadline) {
    // Cancel inside the loop so a long-running zone-active poll is still
    // interruptible from the dashboard. checkCancel throws PipelineCanceled
    // which pipelineWorkerImpl catches and converts to status='canceled'.
    checkCancel(domain)
    try {
...
async function step6GetOrProvisionServer(
  ...
  // Fast path — there's already a ready server we can use. No lock needed.
  if (!forceNew) {
  ...
  // Slow path — serialize across workers so a bulk run doesn't create one
  // droplet PER pipeline when they all hit step 6 simultaneously. The lock
  // holds for the entire provision sequence (DO create + SA install, can
  // take 5-15 min). Late arrivals re-check inside the lock and reuse the
  // newly-ready server instead of provisioning their own. `forceNew=true`
  // still respects operator intent (the re-check is skipped).
  updateStep(domain, 6, "running",
    forceNew
      ? "Operator requested new server — waiting for provision lock..."
      : "No ready server — waiting for provision lock (another pipeline may be provisioning a shared server)...")

  return await withSemaphore("step6-provision", 1, async () => {
    // Re-check inside the critical section regardless of forceNew. The
    // fast-path BEFORE the lock already respected forceNew (skipped the
    // pre-lock reuse), so by the time we're here the operator-supplied
    // "provision new" intent has been honored: the FIRST pipeline in the
    // batch took the lock and provisioned. For subsequent pipelines in the
    // same batch, "force new" almost certainly meant "fresh for the batch",
    // not "fresh for every individual pipeline" — the latter would mean
    // N droplets for N domains, which is exactly the cost the lock was
    // supposed to prevent. So we re-check unconditionally: if a ready
    // server now exists (just provisioned + SA-installed by the previous
    // lock holder), reuse it.
    const reused = findServer(explicitServerId)
    if (reused) {
      updateStep(domain, 6, "completed",
        `Reusing just-provisioned server #${reused.id} ${reused.name} (${reused.ip})  ` +
        `sites=${reused.sites_count}/${reused.max_sites}` +
        (forceNew ? `  (force_new_server honored on first pipeline in batch — sharing thereafter)` : ""))
      setStepArtifact(domain, 6, {
        source: "shared", server_id: reused.id,
        server_name: reused.name, server_ip: reused.ip,
        sa_server_id: reused.sa_server_id,
      })
      return reused
    }
...
async function step7CreateAppAndDns(domain: string, server: ServerRow): Promise<boolean> {
  ...
  // Pre-empt SA's automatic Let's Encrypt: as soon as the app exists, tell
  // SA "no SSL" via POST /ssl/destroy. This stops SA's background workers
  // from issuing an LE cert in the gap between now and step 8. Without
  // this, a leftover `${appName}-le-ssl.conf` shows up in sites-enabled
  // that overrides our CF Origin cert at serving time (operator policy:
  // strictly CF Origin only). Best-effort — a 404 here is the ideal state
  // (no SSL tracker), other failures are non-fatal since step 8's destroy+
  // custom flow handles the same case.
  try {
    const r = await disableAutoSsl(server.sa_server_id!, appId)
    logPipeline(domain, "sa_disable_le", "running",
      `Pre-empted SA auto-LE for app ${appId}: HTTP ${r.status}`)
  } catch (e) {
    logPipeline(domain, "sa_disable_le", "warning",
      `Could not pre-empt SA auto-LE (non-fatal): ${(e as Error).message}`)
  }
...
/**
 * Heuristic: is this LLM error worth retrying, or is it permanent?
 *
 * The provider branches in `lib/website-generator.ts` throw `Error` with
 * messages that typically embed the upstream HTTP status (e.g. "HTTP 401",
 * "HTTP 403") or a known auth/key string. Auth + bad-input errors don't
 * change between attempts — fail fast so the operator gets a clear signal
 * to fix settings instead of waiting for 3 retries × 15s backoff first.
 *
 * NOT matched as permanent: HTTP 429 (rate limit), 5xx, network errors,
 * timeouts — these are exactly the cases the retry loop is here to ride
 * out.
 */
export function isPermanentLlmError(msg: string): boolean {
  const m = String(msg)
  // Treat 400 as permanent too: a malformed-prompt / payload-too-large
  // won't fix itself by retrying. (CF Workers AI pool returns 400 for
  // some quota states, but cf-pool's internal retry already cycled keys
  // BEFORE bubbling up here — so a 400 reaching the pipeline retry layer
  // is a real bad-request.)
  if (/\bHTTP\s*40[013]\b/i.test(m)) return true
  if (/no\s+api\s+key\s+(provided|configured|set)/i.test(m)) return true
  if (/(invalid|bad|expired|missing)\s+api[_\s-]?key/i.test(m)) return true
  if (/authentication[_\s-]?(error|failed)/i.test(m)) return true
  if (/unauthorized|not\s+authorized/i.test(m)) return true
  if (/permission[_\s-]?denied/i.test(m)) return true
...
async function step9GenerateContent(
  ...
  // In-step retry loop so a transient provider failure (502, rate limit,
  // malformed JSON, network blip) doesn't drop the domain to retryable_error
  // and force the auto-heal sweep to pick it up minutes later. Up to
  // LLM_MAX_ATTEMPTS attempts with exponential backoff. ContentBlockedError
  // is terminal — never retried (the model deliberately refused; retrying
  // just burns tokens).
  const LLM_MAX_ATTEMPTS = 3
  const LLM_RETRY_BACKOFF_MS = [5_000, 15_000]
  let lastErr: Error | null = null
  for (let attempt = 1; attempt <= LLM_MAX_ATTEMPTS; attempt++) {
    checkCancel(domain)
    updateStep(domain, 9, "running",
      `Generating single-page site${note}` +
      (attempt > 1 ? ` (retry ${attempt}/${LLM_MAX_ATTEMPTS})` : "...") +
      (attempt > 1 ? `  prev: ${(lastErr?.message ?? "").slice(0, 120)}` : ""))
    try {
      ...
      if (result.usedFallback) {
        ...
        updateStep(domain, 9, "completed",
          ...
          `: ${result.files.map((f) => f.path).join(", ").slice(0, 200)})`)
      } else {
        updateStep(domain, 9, "completed",
          `Generated (niche='${niche}'  bytes=${php.length}` +
          (attempt > 1 ? `  attempt=${attempt}` : "") + `)`)
      }
      ...
      if (e instanceof ContentBlockedError) {
        // Hard refusal from the model — terminal, no point retrying.
      ...
      lastErr = e as Error
      // Fast-fail on permanent errors. Retrying a bad API key or a
      // missing-model error just burns time + cost without changing the
      // outcome — the operator has to fix the credential/setting before
      // ANY attempt can succeed. Domain still flips to retryable_error so
      // auto-heal picks it up after the operator corrects settings.
      if (isPermanentLlmError(lastErr.message)) {
        const reason = lastErr.message.slice(0, 240)
        logPipeline(domain, "llm_gen", "failed",
          `LLM permanent error on attempt ${attempt} — skipping ${LLM_MAX_ATTEMPTS - attempt} ` +
          `remaining retries: ${reason}`)
        updateStep(domain, 9, "failed",
          `LLM permanent error (no retry — fix the API key/provider/model first): ${reason}`)
        updateDomain(domain, { status: "retryable_error" })
        return null
      }
      const delay = LLM_RETRY_BACKOFF_MS[attempt - 1]
      if (delay !== undefined) {
        logPipeline(domain, "llm_gen", "warning",
          `LLM attempt ${attempt}/${LLM_MAX_ATTEMPTS} failed: ${lastErr.message.slice(0, 200)} — ` +
          `retrying in ${Math.round(delay / 1000)}s`)
        await new Promise((res) => setTimeout(res, delay))
        continue
      }
      // Exhausted attempts — surface the final error and let auto-heal
      // (which retries at the pipeline level on a 30-min cooldown) handle it.
      updateStep(domain, 9, "failed",
        `LLM error after ${LLM_MAX_ATTEMPTS} attempts: ${lastErr.message}`)
      updateDomain(domain, { status: "retryable_error" })
      return null
    }
  }
  // Unreachable — the for-loop always returns or throws — but TS doesn't know.
  return null
}
