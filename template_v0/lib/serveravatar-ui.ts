...
export async function installCustomSslViaUi(
  ...
  try {
    ...
    // 1b. SPA hydration warmup. Before this block was added, going straight
    //     from /login to the deep SSL URL produced a 302 back to /login —
    //     evidence: data/sa_ui_debug/*_ssl_page_not_loaded_*.png all show
    //     the login form, not the SSL controls. Hypothesis confirmed by
    //     observation: SA's React dashboard sets sessionStorage with the
    //     current org/server context as the SPA hydrates after login, and
    //     SA's route guards on `/organizations/.../ssl-certificate` reject
    //     requests where that context isn't established. Letting the
    //     dashboard root finish loading first lets the SPA write that
    //     state; the subsequent deep link then passes the guard.
    //
    //     We also visit the org URL explicitly so any per-org context
    //     (current_org_id etc.) gets populated.
    try {
      await page.waitForLoadState("networkidle", { timeout: 15_000 })
    } catch { /* SA may keep a long-poll alive; proceed regardless */ }
    try {
      await page.goto(`${DASHBOARD_URL}/`, { waitUntil: "domcontentloaded", timeout: 30_000 })
      await page.waitForLoadState("networkidle", { timeout: 15_000 })
    } catch { /* dashboard root unreachable shouldn't block — try deep link anyway */ }
    try {
      await page.goto(
    ...
    await snap(page, "post_login_hydrated")
    ...
    // Wait for networkidle so the SPA's route data XHRs finish before we
    // start polling for terminal-state selectors below.
    try {
      await page.waitForLoadState("networkidle", { timeout: 15_000 })
    } catch { /* long-polls — fine */ }
    try {
    ...
    try {
      ...
      // Differentiate "SA kicked us back to login" (session-bound-to-flow
      // problem — operator should use the dashboard's manual "Push cert
      // to SA UI" copy-paste workflow) from "page rendered but selectors
      // changed" (SA UI re-theme — needs code update). The current URL
      // at this point tells us which.
      const url = page.url()
      const kickedToLogin = url.includes("/login")
      throw new SADashboardError(
        kickedToLogin
          ? `SA redirected to login after navigating to SSL URL (session-kick — known SA issue). ` +
            `Use the dashboard's manual "Push cert to SA UI" copy-paste workflow instead. URL=${url}`
          : `SSL page rendered (URL=${url}) but no expected element within 20s ` +
            `(Remove/Installed/Custom tab) — SA may have changed selectors.`,
      )
    }
    await page.waitForTimeout(1500)
