...
export default function WatcherPage() {
  ...
  // up to 2 min while status stays `ns_set` — the old filter was missing
  // those runs entirely and the sidebar said "No active runs"). 2s poll
  // hardcoded — at one operator, the bandwidth cost when idle is trivial
  // and any active-poll adaptiveness would chase its own tail (RUNS now
  // depends on globalWatcher, which depends on poll, which depended on
  // RUNS.length).
  const { watcher: globalWatcher } = useWatcher(2000)
  // `active_domains` = `getAllActiveWatchers()` on the server side =
  // DISTINCT domains with a step_tracker row in 'running' state. This is
  // the authoritative "running RIGHT NOW" source, independent of the
  // domain.status field.
  const activeNames = new Set(globalWatcher?.active_domains ?? [])
  ...
  const RUNS: WatchedRun[] = domains
    ...
    .map((d, i) => ({
      ...
      domain: d.name,
      status: d.status,
      step: d.step || (d.status === "running" || activeNames.has(d.name) ? 5 : 0),
      server: d.server,
      ip: d.ip,
  ...
  // globalWatcher already fetched above (drives both the sidebar's
  // active-domains inclusion AND the cross-domain ACTION REQUIRED scan).
  ...
  async function onForceRerun(stepNum: number, stepName: string) {
    ...
    const baseMsg =
      `Force re-run step ${stepNum} (${stepName}) on ${active.domain}?\n\n` +
    ...
    setPerStepBusy(stepNum); setActionMsg("")
    const r = await domainActions.runFromStep(active.domain, stepNum, { onlyThis: true })
    setActionMsg(r.message ?? r.error ?? `Forced re-run of step ${stepNum}`)
    setPerStepBusy(null)
  }
