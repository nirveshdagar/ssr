...
const CLI_CAPABLE = new Set<string>(["openai", "anthropic_cli"])
  ...
  if (mode === "cli") {
    if (!CLI_CAPABLE.has(provider)) {
      return NextResponse.json({
        ok: false, provider,
        error: `CLI auth is not supported for ${provider} — only openai (codex) or anthropic_cli (claude)`,
      }, { status: 400 })
    }
    const defaultModel =
      provider === "gemini" ? "gemini-2.5-flash" :
      provider === "anthropic_cli" ? "claude-haiku-4-5-20251001" :
      "gpt-5-mini"
