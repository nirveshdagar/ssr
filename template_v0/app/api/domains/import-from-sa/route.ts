...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  for (const sa of saServers) {
    ...
    // Use the repo helper so this insert participates in the gap-fill id
    // allocation (operator preference 2026-05-14). Two-step instead of one
    // raw INSERT, but the cost is negligible at fleet scale.
    const ourId = addServer(name, ipStr, null)
    updateServer(ourId, { sa_server_id: saId, status: "ready" })
