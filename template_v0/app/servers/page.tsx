...
export default function ServersPage() {
  ...
  async function markDead(id: number, name?: string) {
    const label = name ? `"${name}"` : `#${id}`
    if (!confirm(`Mark server ${label} DEAD? Migrate Now is a separate action.`)) return
    setBusy(`d-${id}`)
  ...
  // "Migrate & destroy" — single-button delete that evacuates domains first.
  // Reuses migrate-now; on full migration success `lib/migration.cleanupDeadServer`
  // destroys droplet + SA + DB row automatically (gated by the setting
  // `auto_cleanup_dead_servers`, default on).
  const [evacuateSource, setEvacuateSource] = React.useState<
    { id: number; name: string; ip: string; sites: number } | null
  >(null)
  const [evacuateTarget, setEvacuateTarget] = React.useState<string>("")
  const [evacuateName, setEvacuateName] = React.useState("")
  const [evacuateResult, setEvacuateResult] = React.useState<
    { kind: "ok" | "err"; text: string } | null
  >(null)
  function openEvacuate(s: { id: number; name: string; ip: string; sites: number }) {
    setEvacuateSource(s); setEvacuateTarget(""); setEvacuateName(""); setEvacuateResult(null)
  }
  async function submitEvacuate() {
    ...
    if (evacuateSource.sites === 0) {
      ...
      if (r.ok) {
        ...
        setEvacuateResult({ kind: "err", text: r.error ?? r.message ?? "destroy failed" })
      }
      return
    }
    const target = evacuateTarget && /^\d+$/.test(evacuateTarget) ? Number(evacuateTarget) : undefined
    const r = await serverActions.migrateNow(evacuateSource.id, target)
    if (r.ok) {
      setEvacuateSource(null)
      show("ok",
        `Migration enqueued for ${evacuateSource.sites} domain(s). ` +
        `On full success the server is auto-destroyed ` +
        `(provided Settings → auto_cleanup_dead_servers is on).`)
      await refresh()
    } else {
      setEvacuateResult({ kind: "err", text: r.error ?? r.message ?? "migrate failed" })
    }
  }
  ...
  async function dbDelete(id: number, name?: string) {
    const label = name ? `"${name}"` : `#${id}`
    if (!confirm(`Soft-delete server ${label} from dashboard? DO droplet untouched.`)) return
    setBusy(`x-${id}`)
  ...
  return (
    ...
    >
                      ...
                      <ButtonGroup className="justify-end">
                        ...
                        {s.status === "dead" ? (
                          ...
                          <Button
                            ...
                            onClick={() => markDead(Number(s.id), s.name)}
                          >
                        ...
                        <Button
                          ...
                          onClick={() => dbDelete(Number(s.id), s.name)}
                        >
                        ...
                        <Button
                          ...
                          title="Hard delete — destroy DO droplet + SA record + DB row (typed-name confirm). Refused if any domain still references this server."
                        ...
                        {/* "Migrate & destroy" — one-click delete that evacuates domains
                            to a new server first. Goes through the same migrate-now job
                            as the Migrate icon; auto-destroys the source on full success
                            via cleanupDeadServer. */}
                        <Button
                          variant="ghost" size="icon" className="h-7 w-7 text-status-terminal"
                          aria-label="Migrate domains, then destroy"
                          title={
                            s.domains > 0
                              ? `Migrate all ${s.domains} domain(s) off ${s.name}, THEN destroy droplet + SA + DB row. Typed-name confirm.`
                              : `Destroy ${s.name} (no domains to migrate). Typed-name confirm.`
                          }
                          onClick={() => openEvacuate({
                            id: Number(s.id), name: s.name, ip: s.ip, sites: s.domains,
                          })}
                        >
      ...
      <OperatorDialog
        ...
        title={editName ? `Edit server "${editName}"` : `Edit server #${editingId ?? ""}`}
      ...
      >
        ...
        <Field>
          {/* Server name lives OUTSIDE the FieldLabel because shadcn's Label
              primitive carries `select-none`, making any text inside it
              uncopyable. Pull it into a standalone selectable block + a
              one-click Copy button so the operator can paste it back into
              the confirm input without retyping. */}
          {destroyServer && (
            <div className="flex items-center gap-2 mb-1">
              <span className="text-small text-muted-foreground">Server name:</span>
              <code
                onClick={(e) => {
                ...
                className="font-mono text-status-terminal bg-muted rounded px-1.5 py-0.5 select-all cursor-text"
              >
                {destroyServer.name}
              </code>
              <Button
          ...
          <FieldLabel>Type the server name to confirm</FieldLabel>
          <Input
            value={destroyName}
      ...
      {/* Migrate & destroy modal — evacuates the server's domains via
          migrate-now, then relies on cleanupDeadServer (auto-fired on full
          success when auto_cleanup_dead_servers is on) to destroy the
          droplet + SA + DB row. When domains===0 it falls through to a
          direct hard-delete with the same typed-name confirm. */}
      <OperatorDialog
        open={evacuateSource !== null}
        onOpenChange={(o) => { if (!o) setEvacuateSource(null) }}
        title="Migrate & destroy server"
        description={
          evacuateSource && evacuateSource.sites > 0
            ? "Step 1: migrate every domain off this server to a target (auto-picked unless you choose one). Step 2: on full success, droplet + SA record + DB row are auto-destroyed. Partial migration leaves the server in place so no domain is orphaned."
            : "No domains to migrate — this becomes a direct destroy: droplet + SA record + DB row are removed."
        }
        submitLabel={evacuateSource ? `Migrate & destroy ${evacuateSource.name}` : "Migrate & destroy"}
        submitVariant="destructive"
        onSubmit={submitEvacuate}
        resultMessage={evacuateResult?.text ?? null}
        resultKind={evacuateResult?.kind ?? null}
      >
        {evacuateSource && evacuateSource.sites > 0 && (
          <>
            <div className="rounded-md border border-status-terminal/30 bg-status-terminal/8 px-3 py-2 text-small">
              Move <strong>{evacuateSource.sites}</strong> domain(s) from{" "}
              <code className="font-mono text-foreground">{evacuateSource.name}</code>{" "}
              (<code className="font-mono text-foreground">{evacuateSource.ip}</code>) to a new server,
              then destroy the source.
            </div>
            <ul className="text-micro text-muted-foreground list-disc pl-5 space-y-0.5">
              <li>Uses cached Origin SSL cert (15y) — no CF re-issue</li>
              <li>Uses local content archive — no LLM regen cost</li>
              <li>PATCHes CF A-records directly (nameservers unchanged)</li>
              <li>Auto-cleanup of the source needs Settings → <code className="font-mono">auto_cleanup_dead_servers</code> on (default on)</li>
            </ul>
            <Field>
              <FieldLabel>Target server (optional)</FieldLabel>
              <Select
                value={evacuateTarget || "__auto__"}
                onValueChange={(v) => setEvacuateTarget(v === "__auto__" ? "" : v)}
              >
                <SelectTrigger size="sm" className="h-8 text-small"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="__auto__">Auto — pick ready server or provision fresh droplet</SelectItem>
                  {SERVERS.filter((x) => Number(x.id) !== evacuateSource.id && x.status === "active").map((x) => (
                    <SelectItem key={x.id} value={x.id}>
                      #{x.id} — {x.name} ({x.ip}) · {x.domains}/{x.capacity}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
        ...
        {evacuateSource && (
          <Field>
            ...
            <Input
              value={evacuateName}
              onChange={(e) => setEvacuateName(e.target.value)}
              placeholder={evacuateSource.name}
              className="font-mono"
              autoComplete="off"
            />
            <FieldDescription className="text-status-terminal">
              Destruction is irreversible. On partial migration the server
              stays so you can investigate; rerun the button or use Hard
              Delete after manual cleanup.
            </FieldDescription>
          </Field>
        )}
      </OperatorDialog>
