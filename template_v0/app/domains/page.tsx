...
function DomainsPageInner() {
  ...
  // "Push to SA UI" helper — when SA's REST endpoint refuses with HTTP 500
  // (known bug for some apps) and the patchright UI flow can't keep the SA
  // session across page navigation (bot detection / session-bound-to-IP),
  // we ship a copy-paste workflow instead: this dialog shows the CF Origin
  // cert + key that's currently on the wire, with one-click "Copy" buttons,
  // and a link that opens SA's Custom-SSL page in a new tab. Operator pastes
  // cert + key, clicks Install in SA, SA's tracker updates and the SA UI
  // finally matches what our dashboard says.
  ...
  const [certBundle, setCertBundle] = React.useState<CertBundle | null>(null)
  const [certCopied, setCertCopied] = React.useState<"cert" | "key" | null>(null)
  // Bulk-walk mode — when the operator triggers "Push cert to SA UI" from
  // the bulk-action bar, we keep an ordered queue of remaining domains and
  // advance one at a time. `idx` is the position of the currently-shown
  // bundle. The dialog's Done button advances; the Skip button does the
  // same but logs the skip; closing the dialog aborts the queue.
  const [bulkCertQueue, setBulkCertQueue] = React.useState<{ domains: string[]; idx: number } | null>(null)
  async function openCertBundle(domain: string) {
    setCertBundle({ domain, cert: "", key: "", saUrl: null, loading: true, error: null })
    setCertCopied(null)
    try {
      const r = await fetch(`/api/domains/${encodeURIComponent(domain)}`, { credentials: "same-origin" })
      const j = (await r.json().catch(() => ({}))) as { domain?: { origin_cert_pem?: string; origin_key_pem?: string; server_id?: number } }
      const cert = j.domain?.origin_cert_pem ?? ""
      const key = j.domain?.origin_key_pem ?? ""
      if (!cert || !key) {
        setCertBundle({
          domain, cert: "", key: "", saUrl: null, loading: false,
          error:
            "No CF Origin cert cached on this domain row. Run step 8 first " +
            "(it issues + caches the cert).",
        })
        return
      }
      // Best-effort deep link into SA — we don't always have sa_server_id +
      // sa_app_id wired up in the list response, so when we can build the
      // exact URL we do; otherwise we fall back to SA's root and the
      // operator clicks through.
      const srv = j.domain?.server_id ? serversById[j.domain.server_id] : null
      const orgId = "7254" // primary SA org (from settings); could be made dynamic
      const saUrl = srv?.saServerId
        ? `https://app.serveravatar.com/organizations/${orgId}/servers/${srv.saServerId}/applications/`
        : "https://app.serveravatar.com/"
      setCertBundle({ domain, cert, key, saUrl, loading: false, error: null })
    } catch (e) {
      setCertBundle({
        domain, cert: "", key: "", saUrl: null, loading: false,
        error: `Failed to load cert: ${(e as Error).message}`,
      })
    }
  }
  ...
  // own /api/sa/clear-le-bookkeeping endpoint which runs DELETE /ssl + POST
  // /ssl ssl_type:custom for each domain on the server side. SA's UI updates
  // for domains whose state is non-null. For domains that fail (state is
  // null + LE rate-limited), surface the actual reason so the operator
  // knows it's not a bug they're hitting — it's an upstream SA/LE
  // constraint that no client-side workaround can fix.
  async function startBulkCertPush(): Promise<void> {
    const eligible: string[] = []
    const skipped: string[] = []
    for (const id of selected) {
      const d = DOMAINS.find((x) => x.id === id)
      if (!d) continue
      if (d.sslOk === true) eligible.push(d.name)
      else skipped.push(d.name)
    }
    if (eligible.length === 0) {
      show("err",
        `No selected domain has ssl_origin_ok=1 — nothing to push to SA UI. ` +
        `Run step 8 first.`)
      return
    }
    if (skipped.length > 0) {
      show("ok",
        `Pushing ${eligible.length} domain(s). Skipped ${skipped.length} ` +
        `with no verified CF Origin cert: ${skipped.slice(0, 3).join(", ")}` +
        (skipped.length > 3 ? `, …` : ""))
    }
    setBusy("bulk-cert-push")
    try {
      const r = await fetch("/api/sa/clear-le-bookkeeping", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "same-origin",
        body: JSON.stringify({ domains: eligible }),
      })
      const j = (await r.json().catch(() => ({}))) as {
        ok?: boolean; summary?: { ok: number; skip: number; regressed: number; inconclusive: number; error: number };
        results?: { domain: string; status: string; reason?: string; verify_msg?: string }[]
        error?: string
      }
      if (!r.ok || !j.ok) {
        show("err", j.error ?? `Bulk push failed (HTTP ${r.status})`)
        return
      }
      const okCount = j.summary?.ok ?? 0
      const failed = (j.results ?? []).filter((x) => x.status !== "ok")
      const failedDomains = failed.map((x) => x.domain).join(", ")
      // Surface honest failure shapes — most failures will be the LE-rate-
      // limit / null-state pair. Tell the operator clearly.
      if (failed.length === 0) {
        show("ok", `✓ Pushed CF cert to SA UI for all ${okCount} domain(s).`)
      } else {
        show("err",
          `${okCount}/${eligible.length} pushed. ${failed.length} failed: ${failedDomains.slice(0, 200)}. ` +
          `Most likely SA's POST /ssl is rejecting (HTTP 500) because the app's ssl state is null ` +
          `AND Let's Encrypt is rate-limited (5 duplicate certs / 168h). Wait 5–7 days from the last ` +
          `failed attempt and try again — there's no client-side workaround.`)
      }
    } catch (e) {
      show("err", `Bulk push threw: ${(e as Error).message}`)
    } finally {
      setBusy(null)
    }
  }
  // Keep the manual paste dialog accessible for cases where the operator
  // wants to inspect / hand-paste a single domain's cert (rare, but useful
  // when SA's UI form-submit path is the only one that works for some
  // edge-case state combo we haven't yet diagnosed).
  function startBulkCertWalker(): void {
    const eligible: string[] = []
    for (const id of selected) {
      const d = DOMAINS.find((x) => x.id === id)
      if (d?.sslOk === true) eligible.push(d.name)
    ...
    if (eligible.length === 0) {
      show("err", "No selected domain has ssl_origin_ok=1.")
      return
    }
    setBulkCertQueue({ domains: eligible, idx: 0 })
    void openCertBundle(eligible[0])
  }
  // Advance the queue when the operator clicks Done (or Skip). `skipped`
  // is purely an audit hint — the actual SA install isn't done here, the
  // operator pastes manually. We log it so they have a record of which
  // domains they walked past.
  function advanceBulkCert(skipped: boolean): void {
    if (!bulkCertQueue) {
      setCertBundle(null)
      return
    }
    if (skipped && certBundle?.domain) {
      show("ok", `Skipped ${certBundle.domain}; continuing.`)
    }
    const nextIdx = bulkCertQueue.idx + 1
    if (nextIdx >= bulkCertQueue.domains.length) {
      setBulkCertQueue(null)
      setCertBundle(null)
      show("ok", `Done — walked ${bulkCertQueue.domains.length} domain(s).`)
      return
    }
    setBulkCertQueue({ ...bulkCertQueue, idx: nextIdx })
    void openCertBundle(bulkCertQueue.domains[nextIdx])
  }
  ...
  return (
    ...
    >
                    ...
                    <ButtonGroup className="justify-end">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          ...
                          <DropdownMenuItem
                            onClick={() => openCertBundle(d.name)}
                            title="Open a dialog with the CF Origin cert + key already cached for this domain — copy buttons + a link that opens SA's Custom-SSL page so you can paste in and click Install."
                          >
                            <ClipboardCopy className="mr-2 h-3.5 w-3.5" /> Push cert to SA UI
                          </DropdownMenuItem>
                          {(() => {
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <Button
                size="sm" variant="outline"
                className="gap-1.5 btn-soft-info"
                onClick={startBulkCertPush}
                disabled={busy === "bulk" || busy === "bulk-cert-push"}
                title="Call SA's REST API directly to install our cached CF Origin cert (DELETE /ssl + POST /ssl ssl_type:custom). Updates SA's UI server-side — no manual pasting needed. Reports per-domain success/failure. Domains in null+LE-rate-limited state will fail until LE rate limit refreshes (5-7 days)."
              >
                <ClipboardCopy className="h-3.5 w-3.5" />
                {busy === "bulk-cert-push" ? "Pushing…" : `Push cert to SA UI (${selected.size})`}
              </Button>
              <Button
                size="sm" variant="outline"
                className="gap-1.5"
                onClick={startBulkCertWalker} disabled={busy === "bulk"}
      ...
      {/* Push-cert-to-SA-UI dialog — workaround for SA's HTTP-500 bug + the
          patchright UI session-kick. Shows the CF Origin cert + key currently
          on the wire with one-click Copy buttons, plus a link that opens SA's
          dashboard so the operator can paste cert / key into SA's "Custom
          Installation" form and click Install. After that, SA's tracker
          reflects reality and the SA UI matches the dashboard. */}
      <OperatorDialog
        open={certBundle !== null}
        onOpenChange={(o) => {
          if (!o) {
            // Closing the dialog aborts an in-progress bulk walk so the
            // operator isn't stuck if they decide mid-way to stop.
            setCertBundle(null)
            setBulkCertQueue(null)
          }
        }}
        title={
          bulkCertQueue && certBundle
            ? `Push cert ${bulkCertQueue.idx + 1} of ${bulkCertQueue.domains.length} — ${certBundle.domain}`
            : certBundle
            ? `Push CF Origin cert to SA UI — ${certBundle.domain}`
            : "Push cert to SA UI"
        }
        description={
          "SA's REST API HTTP-500s on these apps (known SA bug), and patchright " +
          "loses the SA session across page navigation. This dialog gives you the " +
          "cached cert + key with one-click Copy buttons so you can paste them into " +
          "SA's Custom Installation form yourself and click Install. After that " +
          "SA's UI will match what's actually serving on origin."
        }
        submitLabel={
          bulkCertQueue
            ? (bulkCertQueue.idx + 1 < bulkCertQueue.domains.length ? "Mark done & next" : "Mark done & finish")
            : "Done"
        }
        onSubmit={() => { advanceBulkCert(false) }}
      >
        {certBundle?.loading && (
          <div className="flex items-center gap-2 text-small text-muted-foreground">
            <Loader2 className="h-4 w-4 animate-spin" /> Loading cert from domain row…
          </div>
        )}
        {certBundle?.error && (
        ...
        {certBundle && !certBundle.loading && !certBundle.error && (
          <>
            <div className="rounded-md border border-border/60 bg-muted/30 px-3 py-2 text-small">
              ...
              <ol className="list-decimal pl-5 space-y-0.5 text-muted-foreground">
                <li>Click <strong>Copy cert</strong> below, paste into SA&apos;s <em>Certificate</em> textarea.</li>
                <li>Click <strong>Copy key</strong>, paste into SA&apos;s <em>Private Key</em> textarea.</li>
                <li>Leave <em>CA Bundle</em> EMPTY (chain breaks mod_ssl when hashes mismatch).</li>
                <li>Turn <strong>Force HTTPS</strong> ON.</li>
                <li>Click <strong>Install Custom Certificate</strong>.</li>
              </ol>
              <div className="mt-2 flex flex-wrap items-center gap-2">
                {certBundle.saUrl && (
                  <a
                    href={certBundle.saUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-2.5 py-1 text-small font-medium hover:bg-muted"
                  >
                    <ExternalLink className="h-3.5 w-3.5" /> Open SA dashboard in a new tab
                  </a>
                )}
                {bulkCertQueue && bulkCertQueue.idx + 1 < bulkCertQueue.domains.length && (
                  <Button
                    type="button" variant="outline" size="sm"
                    onClick={() => advanceBulkCert(true)}
                    title="Don't push this one — jump to the next domain in the queue"
                  >
                    Skip this one →
                  </Button>
                )}
            ...
            <Field>
              <FieldLabel>
                <span className="inline-flex items-center justify-between w-full gap-2">
                  <span>Certificate ({certBundle.cert.length} bytes)</span>
                  <Button
                    type="button" variant="outline" size="sm" className="gap-1.5"
                    onClick={() => copyToClipboard(certBundle.cert, "cert")}
                  >
                    <ClipboardCopy className="h-3.5 w-3.5" />
                    {certCopied === "cert" ? "Copied!" : "Copy cert"}
                  </Button>
                </span>
              </FieldLabel>
              <textarea
                readOnly
                value={certBundle.cert}
                rows={6}
                className="w-full rounded-md border border-border/60 bg-background p-2 font-mono text-micro resize-y"
            ...
            <Field>
              <FieldLabel>
                <span className="inline-flex items-center justify-between w-full gap-2">
                  <span>Private Key ({certBundle.key.length} bytes)</span>
                  <Button
                    type="button" variant="outline" size="sm" className="gap-1.5"
                    onClick={() => copyToClipboard(certBundle.key, "key")}
                  >
                    <ClipboardCopy className="h-3.5 w-3.5" />
                    {certCopied === "key" ? "Copied!" : "Copy key"}
                  </Button>
                </span>
              </FieldLabel>
              <textarea
                readOnly
                value={certBundle.key}
                rows={6}
                className="w-full rounded-md border border-border/60 bg-background p-2 font-mono text-micro resize-y"
                onFocus={(e) => e.currentTarget.select()}
              />
              <FieldDescription className="text-status-waiting">
                Treat this private key like a password. Don&apos;t paste it anywhere except SA&apos;s form.
              </FieldDescription>
            </Field>
          </>
        )}
