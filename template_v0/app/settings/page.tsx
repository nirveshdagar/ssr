              ...
              <Field>
                ...
                <div className="mt-2">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    {/* Probes the locally-installed `claude` binary with the
                        stored OAuth token (Save the token first — the binary
                        reads CLAUDE_CODE_OAUTH_TOKEN from settings, not the
                        unsaved textbox). */}
                    <LlmKeyTestButton
                      provider="anthropic_cli"
                      keyValue={(get("claude_code_oauth_token") as string) ?? ""}
                      cliMode
                      cliBin="claude"
                    />
                  </div>
...
function LlmKeyTestButton({
  ...
  provider: ProviderInfo["id"] | "cloudflare" | "anthropic_cli"
  keyValue: string
  cliMode?: boolean
  cliBin?: string
