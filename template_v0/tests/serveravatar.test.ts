...
describe("serveravatar — deriveSslVhostFromHttp (sshInstallSslFiles recovery)", () => {
  // Real shape from SA's HTTP vhost (`apppilot-site.conf`) on the
  // 2026-05-14 dallas migration, trimmed for the test. Includes the
  // force-https rewrite block our own code injects after step 8's
  // grey-cloud window — derivation must strip it (don't redirect on
  // the HTTPS side).
  const httpVhost = `<VirtualHost *:80>
\tServerAdmin support@serveravatar.com
\tServerName apppilot.site
\tServerAlias apppilot.site www.apppilot.site
\tDocumentRoot /home/apppilotsite/apppilot-site/public_html

\tErrorLog /home/apppilotsite/apppilot-site/logs/error.log
\tCustomLog /home/apppilotsite/apppilot-site/logs/access.log combined

\tIncludeOptional /home/apppilotsite/apppilot-site/conf/apache/*
  ...
  it("converts <VirtualHost *:80> to *:443", async () => {
    const { deriveSslVhostFromHttp } = await import("@/lib/serveravatar")
    const out = deriveSslVhostFromHttp(httpVhost,
      "/etc/ssl/certs/apppilot-site.crt",
      "/etc/ssl/private/apppilot-site.key")
    expect(out).not.toBeNull()
    expect(out).toContain("<VirtualHost *:443>")
    expect(out).not.toContain("<VirtualHost *:80>")
  })

  it("rewrites log paths to *-ssl variants", async () => {
    const { deriveSslVhostFromHttp } = await import("@/lib/serveravatar")
    const out = deriveSslVhostFromHttp(httpVhost,
      "/etc/ssl/certs/x.crt", "/etc/ssl/private/x.key")
    expect(out).toContain("logs/error-ssl.log")
    expect(out).toContain("logs/access-ssl.log")
    expect(out).not.toMatch(/logs\/error\.log/)
    expect(out).not.toMatch(/logs\/access\.log/)
  })

  it("strips the ssr-force-https rewrite block (no redirect loops on HTTPS side)", async () => {
    const { deriveSslVhostFromHttp } = await import("@/lib/serveravatar")
    const out = deriveSslVhostFromHttp(httpVhost, "/c.crt", "/k.key")
    expect(out).not.toContain("ssr-force-https")
    expect(out).not.toContain("RewriteEngine On")
    expect(out).not.toContain("[R=301,L]")
  })

  it("appends SSLEngine + cert paths before </VirtualHost>", async () => {
    const { deriveSslVhostFromHttp } = await import("@/lib/serveravatar")
    const out = deriveSslVhostFromHttp(httpVhost,
      "/etc/ssl/certs/apppilot-site.crt",
      "/etc/ssl/private/apppilot-site.key")
    expect(out).toContain("SSLEngine on")
    expect(out).toContain("SSLCertificateFile /etc/ssl/certs/apppilot-site.crt")
    expect(out).toContain("SSLCertificateKeyFile /etc/ssl/private/apppilot-site.key")
    expect(out).toContain("Protocols h2 http/1.1")
    // SSL block must come BEFORE the closing tag
    const sslPos = out!.indexOf("SSLEngine on")
    const closePos = out!.indexOf("</VirtualHost>")
    expect(sslPos).toBeGreaterThan(0)
    expect(sslPos).toBeLessThan(closePos)
  })

  it("returns null if the HTTP vhost has no *:80 listener", async () => {
    const { deriveSslVhostFromHttp } = await import("@/lib/serveravatar")
    const noVhost = "# just a comment, no VirtualHost\nSomething irrelevant"
    expect(deriveSslVhostFromHttp(noVhost, "/c.crt", "/k.key")).toBeNull()
  })

  it("returns null if there's no </VirtualHost> closing tag", async () => {
    const { deriveSslVhostFromHttp } = await import("@/lib/serveravatar")
    const broken = "<VirtualHost *:80>\n\tServerName x.site\n# missing close"
    expect(deriveSslVhostFromHttp(broken, "/c.crt", "/k.key")).toBeNull()
  })

  it("is idempotent on a vhost that has no force-https block", async () => {
    const minimal = `<VirtualHost *:80>
\tServerName plain.site
\tDocumentRoot /var/www/html
\tErrorLog /home/plain/logs/error.log
</VirtualHost>`
    const { deriveSslVhostFromHttp } = await import("@/lib/serveravatar")
    const out = deriveSslVhostFromHttp(minimal, "/c.crt", "/k.key")
    expect(out).toContain("<VirtualHost *:443>")
    expect(out).toContain("logs/error-ssl.log")
    expect(out).toContain("SSLCertificateFile /c.crt")
  })
})
